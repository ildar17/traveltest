var Article = require('./model/index');
...
exports.list = function (req, res, next) {
   ...
   function listRender() {
      ...
      articleList.list(function (err, result) {
         ...
         articleList.listLimit(limit, offset, function (err, result) {
            if (err) return next(err);
            ...
            if (permission.indexOf('1', 4) == 4) {
               resultList = table.tableListArticle(req, id_one_page, resultList, urlParsed, permission, limit, linkLimit, urlPage, resultLimit);
            }
            ...
            res.render('template/article',
               {
                  ...
                  template: temp,
                  section: section,
                  renew: '<a class="renew_btn" href="' + urlParsed.pathname + '?section='+ sectionOld +'">Вернуться</a>',
...
exports.submit = function ( req, res, next ) {
   ...
   function submitValidate() {
      ...
      if (value.title == ' ' || value.description == ' ' || value.content == ' ' || value.priority == ' ' || value.alias == ' ') {
         ...
         res.error(value.description + ' - должно быть не более 1000 символов!');
         res.repeatData(
            {
               title: value.title,
               alias: value.alias,
         ...
         res.redirect('back');
      } else if (value.priority.length > 10) {
         res.error(value.priority + ' - должно быть не более 10 символов!');
         res.repeatData(
            {
               title: value.title,
               alias: value.alias,
